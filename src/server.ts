...
app.get('/api/quote', (req, res) => {
  ...
  const threshold = Util.parse_uint(req.query.th)
  // Parse the price timestamp.
  const timestamp = Util.parse_uint(req.query.ts) ?? curr_stamp
  ...
  try {
    ...
    const quote = get_price_quote(price_gen, oracle_pk, threshold, timestamp, curr_stamp)
